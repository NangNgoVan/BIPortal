                ...
                using (SqlDataReader reader = DBConnection.command.ExecuteReader())
                {
                    if (reader.HasRows)
                    {
                        while (reader.Read())
                        {
                            ...
                            entityRole.OrganizationName = reader.GetString(reader.GetOrdinal("OrganizationName"));
                            entityRole.DeptId = reader.GetInt32(reader.GetOrdinal("DeptId"));
        ...
        public BlockDataRoleCreateModel GetEntityById(int id)
        {
            ...
            try
            {
                ...
                foreach (DataRow row in table1.Rows)
                {
                    ...
                    output.DeptId = int.Parse(row["DeptId"].ToString());
                }
                //**********************TABLE: ROLEMENU ***********************************************
